...
class RSA
{
    ...
    private function decodeWithKeys($passDecodedBaseSixtyFour)
    {
        
        $passwithoutPublicKey = null;
        $publicKey = null;

        $passLen = strlen($passDecodedBaseSixtyFour) - strlen($this->publicKey);
        
        $chinesseChars = preg_replace(array('/[^\p{Han}？]/u', '/(\s)+/'), array('', '$1'), $passDecodedBaseSixtyFour);

        if ($chinesseChars != '') {
            $passwithoutPublicKey = $chinesseChars;
        } else {
            $passDecodedBaseSixtyFour = $this->replaceEspecialChars($passDecodedBaseSixtyFour);

            if ($passLen > strlen($this->publicKey)) {
                $limitPublicKey = strlen($this->publicKey) * 2;
                
                for ($i = 0; $i < strlen($passDecodedBaseSixtyFour); $i++) {
                    if ($i < $limitPublicKey) {
                        if ($i % 2 == 0) {
                            $publicKey .= $passDecodedBaseSixtyFour[$i];
                        } else {
                            $passwithoutPublicKey .= $passDecodedBaseSixtyFour[$i];
                        }
                    } else {
                        $passwithoutPublicKey .= $passDecodedBaseSixtyFour[$i];
                    }
