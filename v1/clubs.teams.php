...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    ...
    if ( is_null($httpResponse)) {
        ...
        $paramsReceived = json_decode($params, true);        

        if (isset($paramsReceived['category_id']) || isset($paramsReceived['division_id'])){
            $httpResponse = $teamController->getTeamsWithFilters($params);
        }else{
            $httpResponse = $clubController->getClubsWithFilters($params);
        }        
